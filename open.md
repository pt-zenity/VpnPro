sudo nano /root/install-openvpn-xor-v273-final.sh
...
read -r -p "Enter server domain or IP, example vpn.example.com: " SERVER_HOST
read -r -p "Enter first client name, example client1: " FIRST_USER
...
2. Запуск установки
sudo chmod +x /root/install-openvpn-xor-v273-final.sh
sudo bash /root/install-openvpn-xor-v273-final.sh

Когда спросит:

Enter server domain or IP:

вводишь домен, например:

harmany.shop

Когда спросит:

Enter first client name:

вводишь:

clienttun
3. Проверка после установки
cd /root/ovpn-xor-admin
./check.sh
./status.sh

Должно быть примерно так:

OK: no unrecognized scramble error
active
udp 0.0.0.0:443
4. Где забрать конфиг через Termius SFTP

Скрипт сам копирует .ovpn сюда:

/home/ubuntu/ovpn-clients/

Например:

/home/ubuntu/ovpn-clients/clienttun.ovpn

Проверить:

ls -la /home/ubuntu/ovpn-clients/
5. Создать нового пользователя
cd /root/ovpn-xor-admin
./add-user.sh client2

Файл появится сразу в двух местах:

/root/ovpn-xor-admin/clients/client2.ovpn
/home/ubuntu/ovpn-clients/client2.ovpn

Через Termius SFTP забирай:

/home/ubuntu/ovpn-clients/client2.ovpn
6. Если нужно вручную экспортировать конфиг в папку ubuntu
sudo export-ovpn clienttun

После этого файл будет тут:

/home/ubuntu/ovpn-clients/clienttun.ovpn
7. Отозвать пользователя
cd /root/ovpn-xor-admin
./revoke-user.sh client2

Это:

отзовёт сертификат
обновит CRL
удалит .ovpn из /root/ovpn-xor-admin/clients
удалит .ovpn из /home/ubuntu/ovpn-clients
перезапустит OpenVPN
...
systemctl status openvpn-xor --no-pager
ss -lunpt | grep ':443'
tail -100 /var/log/openvpn-xor.log
10. Backup
cd /root/ovpn-xor-admin
./backup.sh

Скачать backup через SFTP сложнее, потому что он лежит в /root. Можно скопировать его в /home/ubuntu:

sudo cp /root/openvpn-xor-backup-*.tar.gz /home/ubuntu/
sudo chown ubuntu:ubuntu /home/ubuntu/openvpn-xor-backup-*.tar.gz

Потом через Termius SFTP забрать из:
